...
fn process_card(card_matches: &mut HashMap<usize, usize>, copies: &mut HashMap<usize, usize>, index: usize, card: &str) -> usize {
    let points: usize = if let Some(points) = card_matches.get(&index) {
        *points
    } else {
        ...
        actual.split_whitespace().fold(0, |acc, n| {
    ...
    card_matches.insert(index, points);
...
impl Solution for Day4B {
    fn run(&self, input: &str) -> Result<String, anyhow::Error> {
        ...
        for (i, c) in cards.into_iter().enumerate() {
            let points = process_card(&mut card_matches, &mut copies, i, c);
            let n_copies = *copies.get(&i).unwrap_or(&0);
            if points > 0 {
                for _ in 0..n_copies {
                    process_card(&mut card_matches, &mut copies, i, c);
                }
            }
